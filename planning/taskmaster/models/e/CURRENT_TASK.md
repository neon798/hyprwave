...
- [ ] Audit `hyprlock.conf` + `hypridle.conf`: lock keybind exists, idle → lock → DPMS chain documented; no broken paths
- [ ] Document theme symlink layout (`~/.config/hyprwave/theme` and per-app links) in `planning/integration/e-hyprland/THEME-SYMLINKS.md`; fix broken skel links if any
- [ ] `monitors.conf`: sensible default + comments for multi-monitor / scaling; do not hardcode one user’s layout as silent default if risky
- [ ] Walker/mako/waybar: confirm theme-friendly paths; no stale wofi/swaybg
- [ ] Expand SESSION-SMOKE.md with lock/idle/theme-switch checks (≥5 new items)
- [ ] KEYBIND-MAP.md regenerated/updated if binds change
- [ ] HANDOFF.md: any package gaps (hyprshot, etc.) listed for integrator
