...
Freeze tip: D-W1-006 `adbb4f4` — duress packaging frozen; PAM OFF.

## Rules

1. Refresh taskmaster from `origin/main` each poll.
2. **Do not** set `status: DONE` while `task_id` is still `D-W1-HOLD`.
...
See IDENTITY.md (product freeze) + `planning/taskmaster/models/d/**` for logs only.
